...
for i in 0...supports.length
  fall = Set.new [i]
  for j in i+1...supports.length
    fall << j if !supports[j].empty? && supports[j].all? { |s| fall.include?(s) }   # !supports[j].empty? -> this condition's been a nightmare
    # puts [supports[j], fall].to_s
  end
